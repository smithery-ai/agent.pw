...
const HOST_ICON_OVERRIDES: HostIconOverride[] = [
  ...
  { pattern: /(^|\.)openai\.com$/i, iconHost: 'openai.com', fallback: 'OA' },
  {
    ...
    iconHost: 'anthropic.com',
    fallback: 'AN',
  },
  {
    pattern: /(^|\.)googleapis\.com$/i,
    iconHost: 'google.com',
    fallback: 'GO',
  },
  {
    pattern: /(^|\.)atlassian\.net$/i,
    iconHost: 'atlassian.com',
    fallback: 'AT',
  },
]
...
function defaultMonogram(hostname: string, displayName?: string) {
  ...
  const core = normalizeHostname(hostname).replace(
    /^(api|www|app|gateway|m)\./,
    '',
  )
...
function findOverride(hostname: string) {
  ...
  return HOST_ICON_OVERRIDES.find((override) =>
    override.pattern.test(normalized),
  )
}

export function inferServiceIconPreview(
  hostname: string,
  displayName?: string,
): ServiceIconPreview {
  const normalized = normalizeHostname(hostname)
  ...
  const iconHost =
    override?.iconHost ??
    normalized.replace(/^(api|www|app|gateway|m)\./, '')
  const fallback =
    override?.fallback ?? defaultMonogram(normalized, displayName)
