  ...
  switch (command) {
    ...
    case 'creds': {
      ...
      if (subcommand === 'add') {
        ...
        const credentialName = nameIndex !== -1 ? args[nameIndex + 1] : undefined
        ...
        if (!target) {
          console.error('Usage: agent.pw cred add <profile-or-host> [--value <key>] [--name <credential-name>] [--auth headers] [-H "Header: value"]')
          process.exit(1)
        }
        const { addCred } = await import('./commands/cred')
        return addCred(target, value, { auth, credentialName, headers })
      }
      if (subcommand === 'remove' || subcommand === 'rm') {
        const name = args[2]
        if (!name) {
          console.error('Usage: agent.pw cred remove <name> [--host <hostname>] [--path <credential-path>]')
          process.exit(1)
        }
        const hostIndex = args.indexOf('--host')
        const host = hostIndex !== -1 ? args[hostIndex + 1] : undefined
        ...
        return removeCred(name, { host, path })
      }
    ...
    default: {
      ...
      console.log('  cred add <profile-or-host>      Add a credential')
      console.log('  cred remove <name>              Remove a credential')
