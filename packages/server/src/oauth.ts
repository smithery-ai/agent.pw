...
async function resolveAuthorizationServer(
  ...
  if (!(config.authorizationUrl && config.tokenUrl)) {
    throw new AgentPwInputError('OAuth configuration requires either issuer or authorizationUrl + tokenUrl')
  }
...
function parseProfileOAuthConfig(
  ...
  const clientId = profile.auth.clientId ?? clientInput?.clientId ?? clientInput?.metadata?.clientId
  ...
  if (!clientId) {
    throw new AgentPwInputError(`Credential Profile '${profile.path}' requires a clientId or default oauth client`)
  }
...
async function resolveOAuthConfigForResourceOption(
  ...
  if (!client) {
    throw new AgentPwInputError(`Resource '${option.resource}' requires oauth client configuration`)
  }
  ...
  if (!issuer) {
    throw new AgentPwInputError(`Resource '${option.resource}' does not advertise an authorization server`)
  }

  if (option.authorizationServer && !discovered.authorizationServers.includes(option.authorizationServer)) {
  ...
  let clientAuthentication = normalizeClientAuthentication(client.clientAuthentication, Boolean(clientSecret))
  ...
  if (!clientId) {
    throw new AgentPwInputError(`Resource '${option.resource}' requires a clientId or dynamic client registration`)
  }
...
export function createOAuthService(options: {
  ...
  async function resolveOAuthConfigForOption(
    ...
    if (option.source === 'profile') {
      ...
      return parseProfileOAuthConfig(profile, option.resource, clientInput ?? options.defaultClient)
    }

    return resolveOAuthConfigForResourceOption(option, clientInput ?? options.defaultClient, options.customFetch)
  }
  ...
  return {
    ...
    async startAuthorization(input: ConnectStartInput): Promise<ConnectAuthorizationSession> {
      ...
      if (!authorizationServer.authorization_endpoint) {
        throw new AgentPwInputError(`OAuth option for '${input.option.resource}' is missing an authorization endpoint`)
      }
      ...
      if (credential.auth.kind === 'oauth') {
        ...
        if (oauthConfig) {
          ...
          if (authorizationServer.revocation_endpoint) {
            ...
            if ((revokeMode === 'refresh_token' || revokeMode === 'both') && credential.secret.oauth?.refreshToken) {
            ...
            if ((revokeMode === 'access_token' || revokeMode === 'both') && credential.secret.oauth?.accessToken) {
              const response = await oauth.revocationRequest(
                authorizationServer,
                client,
                clientAuthentication,
