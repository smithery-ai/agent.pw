...
function createOAuthFetch() {
	...
	const fetchImpl: typeof fetch = async (input, init) => {
		...
		if (
			url ===
				"https://auth.docs.example.com/.well-known/oauth-authorization-server" ||
...
describe("oauth runtime", () => {
	it("runs profile-backed oauth end to end and refreshes stale credentials", async () => {
		...
		expect(session.authorizationUrl).toContain(
			"resource=https%3A%2F%2Fapi.linear.app%2Fprojects",
		);
	...
	it("runs discovery-first oauth, hosted handlers, and cimd helpers", async () => {
		...
		expect(session.authorizationUrl).toContain(
			"https://auth.docs.example.com/authorize",
		);
		expect(session.authorizationUrl).toContain(
		...
		expect(cimdResponse.headers.get("cache-control")).toBe(
			"public, max-age=300",
		);
		expect(calls.map((call) => call.url)).toEqual(
			expect.arrayContaining([
	...
	it("persists challenge-origin oauth flow semantics inside agent.pw", async () => {
		...
		expect(started.kind).toBe("authorization");
		if (started.kind !== "authorization") {
			throw new Error("Expected authorization start");
		}
