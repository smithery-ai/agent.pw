...
function createDiscoveryFetch(options: {
  ...
  const fetchImpl: typeof fetch = async input => {
    ...
    if (
      ...
      return Response.json({
        ...
        ...(includeRegistrationEndpoint ? { registration_endpoint: 'https://auth.example.com/register' } : {}),
      })
    }
...
describe('oauth edge cases', () => {
  it('requires explicit flow storage and valid oauth options', async () => {
    ...
    await expect(headersOnly.connect.start({
    ...
    await expect(agentPw.connect.start({
      path: '/org/connections/docs',
      option: {
        kind: 'oauth',
        source: 'discovery',
      ...
      redirectUri: 'https://app.example.com/oauth/callback',
    })).rejects.toThrow("Resource 'https://docs.example.com/mcp' requires oauth client configuration")
  ...
  it('validates missing state, unknown flows, expired flows, and profile oauth config errors', async () => {
    ...
    await expect(agentPw.connect.complete({
  ...
  it('handles discovery fallback, refresh edge cases, and disconnect behavior', async () => {
    ...
    expect(await failingDiscovery.connect.prepare({
    ...
    expect(await agentPw.connect.disconnect({ path: '/org/connections/oauth-no-revoke', revoke: 'both' })).toBe(true)
  })
  ...
  it('supports global fetch discovery, custom hosted responses, and cimd validation', async () => {
    ...
    const handlers = agentPw.connect.createWebHandlers({
      ...
      error(error) {
        return Response.json({ error: error instanceof Error ? error.message : 'unknown' }, { status: 418 })
      },
    })
    ...
    const success = await handlers.callback(new Request(
      `https://app.example.com/oauth/callback?code=code-123&state=${flowId}`,
    ))
    expect(success.status).toBe(201)
    expect(await success.json()).toEqual({ path: '/org/connections/docs' })

    const failure = await handlers.callback(new Request('https://app.example.com/oauth/callback?code=missing'))
  ...
  it('surfaces discovery option validation and dynamic registration errors', async () => {
    ...
    await expect(agentPw.connect.start({
      ...
      option,
      redirectUri: 'https://app.example.com/oauth/callback',
    })).rejects.toThrow("Authorization server 'https://auth.example.com' does not support dynamic client registration")
    ...
    await expect(agentWithFixedClient.connect.start({
      path: '/org/connections/docs',
      option: {
        kind: 'oauth',
        source: 'discovery',
