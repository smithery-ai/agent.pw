...
function createDiscoveryFetch(
	...
	const includeRegistrationEndpoint =
		options.includeRegistrationEndpoint ?? true;
...
describe("oauth edge cases", () => {
	...
	it("validates missing state, unknown flows, expired flows, and profile oauth config errors", async () => {
		...
		await expect(
			agentPw.connect.complete({
				callbackUri:
					"https://app.example.com/oauth/callback?code=expired&state=expired-state",
			}),
		...
		await expect(
			...
			"Credential Profile '/broken' requires a clientId or default oauth client",
		);
	});
	...
	it("handles discovery fallback, refresh edge cases, and disconnect behavior", async () => {
		...
		await agentPw.profiles.put("/docs-api", {
			...
			auth: {
				kind: "headers",
				fields: [
					{ name: "Authorization", label: "Bearer token", prefix: "Bearer " },
				],
			},
		...
		expect(
			await agentPw.connect.headers({ path: "/org/connections/manual" }),
		).toEqual({
	...
	it("supports global fetch discovery, custom hosted responses, and cimd validation", async () => {
		...
		const option = prepared.options.find(
		...
		const startResponse = await handlers.start(
			new Request("https://app.example.com/connect"),
			{
				path: "/org/connections/docs",
				option,
			},
