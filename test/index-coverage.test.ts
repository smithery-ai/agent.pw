...
describe('index coverage helpers', () => {
  it('rejects malformed auth payloads and invalid json writes', async () => {
    ...
    expect(await agentPw.profiles.get('/minimal-oauth')).toEqual(expect.objectContaining({
      auth: {
        kind: 'oauth',
        label: undefined,
        issuer: undefined,
    ...
    expect(await agentPw.profiles.get('/oauth-labeled')).toEqual(expect.objectContaining({
    ...
    await expect(Reflect.apply(agentPw.profiles.put, agentPw.profiles, ['/invalid-json', {
    ...
    `))
    await expect(agentPw.profiles.get('/broken-profile')).rejects.toThrow('Invalid profile auth payload')
    ...
    `))
    expect(await agentPw.profiles.get('/field-fallback')).toEqual(expect.objectContaining({
      auth: {
        kind: 'headers',
        label: undefined,
        fields: [{ name: 'Authorization', label: 'Token', description: undefined, prefix: undefined, secret: undefined }],
      },
    }))
    ...
    expect(await agentPw.profiles.get('/headers-rich')).toEqual(expect.objectContaining({
      auth: expect.objectContaining({
        label: 'Headers',
        fields: [{ name: 'Authorization', label: 'Token', description: 'Describe', prefix: 'Bearer ', secret: true }],
      }),
    ...
    `))
    expect(await agentPw.profiles.get('/field-empty')).toEqual(expect.objectContaining({
      auth: {
        kind: 'headers',
        label: undefined,
        fields: [],
    ...
    `))
    await expect(agentPw.credentials.get('/broken/credential')).rejects.toThrow('Invalid credential auth payload')
    ...
    await expect(agentPw.credentials.list({ path: '/../bad' })).rejects.toThrow("Invalid credential path '/../bad'")
    await expect(agentPw.profiles.list({ path: '/../bad' })).rejects.toThrow("Invalid profile path '/../bad'")
  })
  ...
  it('covers the authorized facade across connect, credentials, and profiles', async () => {
    ...
    const result = await (async () => {
      ...
      await expect(api.connect.complete({
    ...
    expect(result.listedCredentials.map(credential => credential.path)).toEqual(['/acme/connections/linear'])
