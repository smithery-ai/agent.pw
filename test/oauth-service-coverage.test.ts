...
describe("oauth service coverage", () => {
  ...
  it("sends unreserved client_secret_basic credentials without percent-encoding", async () => {
    const state = await createState();
    const flowStore = createInMemoryFlowStore();
    let decodedAuthorization: string | undefined;
    ...
    const fetchImpl: typeof fetch = async (input, init) => {
      ...
      if (
        url === "https://issuer-basic.example.com/.well-known/oauth-authorization-server" ||
        url === "https://issuer-basic.example.com/.well-known/openid-configuration"
      ) {
        return Response.json({
          issuer: "https://issuer-basic.example.com",
          authorization_endpoint: "https://issuer-basic.example.com/authorize",
          token_endpoint: "https://issuer-basic.example.com/token",
          code_challenge_methods_supported: ["S256"],
        });
      }

      if (url === "https://issuer-basic.example.com/token") {
        const authorization = new Headers(init?.headers).get("authorization");
        decodedAuthorization = Buffer.from(
    ...
    await state.profiles.set("basic-client", {
      path: "basic-client",
      resourcePatterns: ["https://issuer-basic.example.com/*"],
      http: null,
      oauth: {
        issuer: "https://issuer-basic.example.com",
    ...
    const started = await service.startAuthorization({
      path: "org.basic-client",
      option: {
        kind: "oauth",
        source: "profile",
        label: "Basic client",
        profilePath: "basic-client",
        resource: "https://issuer-basic.example.com/api",
      },
      redirectUri: "https://app.example.com/oauth/callback",
    });

    await service.completeAuthorization({
      callbackUri: `https://app.example.com/oauth/callback?code=code&state=${started.flowId}`,
    });

    expect(decodedAuthorization).toBe("registered-client-id:registered-client-secret");
  });

  it("keeps strict client_secret_basic encoding when any credential is reserved", async () => {
    const state = await createState();
    const flowStore = createInMemoryFlowStore();
    let decodedAuthorization: string | undefined;
    ...
    const fetchImpl: typeof fetch = async (input, init) => {
      ...
      if (
        url === "https://issuer-strict-basic.example.com/.well-known/oauth-authorization-server" ||
        url === "https://issuer-strict-basic.example.com/.well-known/openid-configuration"
      ) {
        return Response.json({
          issuer: "https://issuer-strict-basic.example.com",
          authorization_endpoint: "https://issuer-strict-basic.example.com/authorize",
          token_endpoint: "https://issuer-strict-basic.example.com/token",
          code_challenge_methods_supported: ["S256"],
        });
      }

      if (url === "https://issuer-strict-basic.example.com/token") {
        const authorization = new Headers(init?.headers).get("authorization");
        decodedAuthorization = Buffer.from(
          authorization?.replace(/^Basic /, "") ?? "",
          "base64",
        ...
        return Response.json({
          access_token: "strict-basic-access",
          refresh_token: "strict-basic-refresh",
          token_type: "Bearer",
        });
      }
    ...
    await state.profiles.set("strict-basic-client", {
      path: "strict-basic-client",
      resourcePatterns: ["https://issuer-strict-basic.example.com/*"],
      http: null,
      oauth: {
        issuer: "https://issuer-strict-basic.example.com",
        clientId: "registered-client-id",
    ...
    const started = await service.startAuthorization({
      path: "org.strict-basic-client",
      option: {
        kind: "oauth",
        source: "profile",
        label: "Strict basic client",
        profilePath: "strict-basic-client",
        resource: "https://issuer-strict-basic.example.com/api",
      },
      redirectUri: "https://app.example.com/oauth/callback",
    });
    ...
    expect(decodedAuthorization).toBe("registered%2Dclient%2Did:registered%2Dclient%2Dsecret%21");
  });
